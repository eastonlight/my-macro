...
impl Labels {
    ...
    pub const fn english() -> Self {
        Self {
            ...
            spire_target_name: "Spire (3x3)",
            row_build_hint: "The one trigger key runs this macro. The checkbox above picks the building (Creep Colony or Spire) for the shown build sequence and the shared timing. It counts the selected drones (2-12) and builds a row to the right of the cursor, with the row order set to left to right or ends inward (both cover the same span, the cursor staying the leftmost footprint). Spacing follows the building: Creep Colony 2 tiles (144 px), Spire 3 tiles (216 px). The Spire spacing is inferred from tile math and is NOT live-verified yet; with the forced mode off, a failed green preview check stops the run instead of clicking. Uses scratch group 9, 1920x1080 HUD only. The count shown is orders issued, not finished buildings.",
